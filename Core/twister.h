...
public:
	AllTwister(
		...
		: table(index_space * Twists::all().size())
	{
		const int64_t size = static_cast<int64_t>(index_space);
		#pragma omp parallel for
		for (int64_t i = 0; i < size; i++)
			for (Twist t : Twists::all())
				table[i * Twists::all().size() + std::to_underlying(t)] = index(from_index(i).twisted(t));
	}

	index_t operator()(uint64_t i, Twist t) const
	{
		return table[i * Twists::all().size() + std::to_underlying(t)];
	}
};
